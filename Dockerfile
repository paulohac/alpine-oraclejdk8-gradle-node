...
LABEL maintainer "Paulo Cardoso" "<paulohenrique@softbox.com.br>"
ENV JAVA_VERSION=8  \
    JAVA_UPDATE=151 \
    JAVA_BUILD=12 \
    ...
    PATH="$PATH:$JAVA_HOME:/opt/gradle-3.3/bin"
RUN apk upgrade --update && \
    apk add --no-cache --virtual=build-dependencies bash libstdc++ curl ca-certificates unzip && \
    ...
    apk update && apk add zip && apk add openssl && \
